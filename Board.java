 ...
 */
 private Board(Board prevBoard, int columnToDrop) {
     ...
     if (prevBoard.isColumnFull(columnToDrop))
         throw new IllegalArgumentException("Board is full in column " + 
                                            columnToDrop);
     // copy the board
        ...
        board[rowOfNewToken][columnToDrop] = (byte) 
(prevBoard.getPlayerToMoveNext() == Player.MAX ? 1 : -1);
        ...
        for (int r = 0; r < numRows; r++) {
            for (int c = 0; c < numCols; c++) {
                ...
                if ((c <= numCols - consecToWin && allMatchInARow(r, c))
                    ...
                    || (r <= numRows - consecToWin && c <= numCols - 
                                                           consecToWin
 ...
 */
 ...
 public Player getPlayerToMoveNext() {
    ...
    /**
 * Return the state of the game (in progress, win for max, win for min, or 
tie).
 */
 ...
 */
 public String toString() {
    ...
    // return number of consec-in-a-row - I don't know if this works'
 public int containNInARow(int consec, int currPlayer) {
     int count = 0;
     for (int r = 0; r < numRows; r++) {
         for (int c = 0; c < numCols; c++) {
             if (board[r][c] == 0) {
             ...
             if ((c <= numCols - consec && numMatchInARow(r, c, consec, currPlayer))
                 || (r <= numRows - consec && numMatchInAColumn(r, c, consec, currPlayer))
                 || (r <= numRows - consec && c <= numCols - consec
                     && numMatchInANorthEastDiagonal(r, c, consec, currPlayer))
                 || (r <= numRows - consec && c - consec >= -1 && 
                     numMatchInANorthWestDiagonal(r, c, consec, currPlayer))) {
                 count++;
             }
         }
     }
 ...
 private boolean allMatchInARow(int row, int startcol) {
    ...
    private boolean allMatchInANorthEastDiagonal(int startrow, int startcol) {
     for (int x = 0; x < consecToWin - 1; x++) {
         if (board[startrow + x][startcol + x] != board[startrow + x + 1]
[startcol + x + 1])
             return false;
     }
        return true;
...
[startcol - x - 1])
    ...
    ////////////// number of in a row for the board
 //distinguish between players
 private boolean numMatchInARow(int row, int startcol, int numMatch, int player)
{
    for (int x = 0; x < numMatch - 1; x++) {
        // if board[r][c] != player, false
        if (board[row][startcol + x] != (byte) player
            || board[row][startcol + x] != board[row][startcol + x + 1]) {
            return false;
        }
    }
    return true;
}
    private boolean numMatchInAColumn(int startrow, int col, int numMatch, int 
player) {
     for (int x = 0; x < numMatch - 1; x++) {
         if (board[startrow + x][col] != (byte) player || board[startrow + x]
[col] != board[startrow + x + 1][col])
             return false;
     }
        return true;
 }
    private boolean numMatchInANorthEastDiagonal(int startrow, int startcol, int 
numMatch, int player) {
     for (int x = 0; x < numMatch - 1; x++) {
         if (board[startrow + x][startcol + x] != (byte) player
             || board[startrow + x][startcol + x] != board[startrow + x + 1]
[startcol + x + 1])
             return false;
     }
        return true;
 }
    private boolean numMatchInANorthWestDiagonal(int startrow, int startcol, int 
numMatch, int player) {
     for (int x = 0; x < numMatch - 1; x++) {
         if (board[startrow + x][startcol - x] != (byte) player
             || board[startrow + x][startcol - x] != board[startrow + x + 1]
